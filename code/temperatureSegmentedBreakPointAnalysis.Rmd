  ...
  for (year in unique(e1$year)){
    ...
    if(numBeforeBP2 > 0 & numAfterBP2 > 0){
      ...
      if(exists('out')){
        ...
        if( out$coeff[2] < 0 | out$coeff[3] > 0 ){
        ...
        if(numBeforeBP2 >= numBeforeCompleteBP2 & numAfterBP2 >= numAfterCompleteBP2 & out$coeff[2] > 0 & out$coeff[3] < 0 ){
          ...
          breaks$summerBPSigma   [breaks$year == year & breaks$site == siteList[j]] <- Sigma
      ...
      title(main=paste(e1$site[1],"_",year, "_", Sigma))
      dev.off()
...
for (j in 1:nSites){
  ...
  for (year in unique(e1$year)){
    ...
    #If the number of points before and after the estimated mean summer breakpoint exists, proceed to calculate site specific breakpoint.
      ...
      if(exists('out')) {curTitle <-  paste0(e1$site[1],"_",year, "_", Sigma)} else(curTitle <-  paste0(e1$site[1],"_",year, "_NA") )
