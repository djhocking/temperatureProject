...
et <- et[,-which(names(et) %in% c('Latitude', 'Longitude'))]
...
lme2 <- lme(temp~airTemp+airTempLagged1+airTempLagged2+
              LatitudeS+LongitudeS+
              ForestS+ AgricultureLS+ 
              BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
              WetlandOrWaterS+ SurficialCoarseCLS+ImpoundmentsOpenSqKMLS+ 
              daylS + sradS + sweLS, random = ~1)

lmeTest <- lme(temp ~ airTemp+airTempLagged1+airTempLagged2, random = ~ 1|site, data = et, method = "ML", na.action = "na.omit") # |year/site ??? +
                 LatitudeS+LongitudeS+
                 ForestS+ AgricultureLS+ 
                 BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
                 WetlandOrWaterS+ SurficialCoarseCLS+ImpoundmentsOpenSqKMLS+ 
                 daylS + sradS + sweLS +
                 sin(2*pi/360*dOY) + cos(2*pi/360*dOY)
summary(lmeTest)
...
lmefull <- lme(temp ~ airTemp+airTempLagged1+airTempLagged2+
                 LatitudeS+LongitudeS+
                 ForestS+ AgricultureLS+ 
                 BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
                 WetlandOrWaterS+ SurficialCoarseCLS+ImpoundmentsOpenSqKMLS+ 
                 daylS + sradS + sweLS +
                 sin(2*pi/360*dOY) + cos(2*pi/360*dOY), random = ~ sin(2*pi/360*dOY) + cos(2*pi/360*dOY) + airTemp + airTempLagged1 + airTempLagged2 + ForestS + AgricultreLS|site, data = et, method = "ML", na.action = "na.omit") # |year/site ???
...
if(validate){
  ...
  modelMetricsS2$Sigma <- c(summary(m0S2)$sigma, summary(m1S2)$sigma,summary(m2S2)$sigma,summary(m3S2)$sigma,summary(m4S2)$sigma)
...
if(validate){
  ...
  modelMetricsS3$Sigma <- c(summary(m0S3)$sigma, summary(m1S3)$sigma,summary(m2S3)$sigma,summary(m3S3)$sigma,summary(m4S3)$sigma)
  modelMetricsS3
