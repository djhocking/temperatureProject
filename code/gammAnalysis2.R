...
system.time(bam6 <- bam(temp ~ airTemp + airTempLagged1+airTempLagged2+ prcp + prcpLagged1 + prcpLagged2 +  airTemp*prcp + s(dOY, by = fsite) + s(fsite, airTemp, bs = 're') + s(fyear, airTemp, bs = 're'), data = etS))
summary(bam6) # r2 = 97.6 fSeason + airTemp*fSeason +
plot(bam6)
plot(etS$date, etS$temp, cex = 0.1, pch = 16, ylim = c(-5, 25)) 
I <- order(etS$date)
