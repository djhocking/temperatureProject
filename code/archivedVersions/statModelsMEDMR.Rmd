...
et2$daylS <- (et2$dayl-mean(et2$dayl,na.rm=T))/sd(et2$dayl,na.rm=T)#scale(et$dayl)
et2$sradS <- (et2$srad-mean(et2$srad,na.rm=T))/sd(et2$srad,na.rm=T)#scale(et$srad)
et2$sweLS  <- (et2$sweL-mean(et2$sweL,na.rm=T))/sd(et2$sweL,na.rm=T)#scale(et$swe)


#don't use "scale" because it creates a data type with attributes that make is difficult to use predict
et3$daylS <- (et3$dayl-mean(et3$dayl,na.rm=T))/sd(et3$dayl,na.rm=T)#scale(et$dayl)
et3$sradS <- (et3$srad-mean(et3$srad,na.rm=T))/sd(et3$srad,na.rm=T)#scale(et$srad)

#Make sure all values aren't zero if you want to use this in stats models:
et3$sweLS <-   (et3$sweL-mean(et3$sweL,na.rm=T))/sd(et3$sweL,na.rm=T)#scale(et$swe)
...
m7bp1 <- lmer(springBP~
              ...
              ForestS+ AgricultureLS+ 
              BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
              WetlandOrWaterS+ SurficialCoarseCS +
...
#--------------------------------------------------------------
UpstreamStats$sweL <- log(UpstreamStats$swe + 0.001)
...
for ( i in 1:length(DaymetTiles)){
  ...
  test <- merge(FullRecord, FRday, by = c('site', 'year.x', 'dOY'), all.x = T, sort = F)
