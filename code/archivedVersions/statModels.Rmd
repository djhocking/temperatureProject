...
m3S2 <- lm(temp~airTemp + airTempLagged1 + airTempLagged2 + Latitude + Longitude + BasinElevationM + ReachSlopePCNT + 
     ...
     airTemp*log(ImpoundmentsOpenSqKM+0.001) + airTemp*log(ImpoundmentsOpenSqKM+0.001) + airTemp*sweLS + 
     
     airTempLagged1 + airTempLagged1*ReachSlopePCNT + airTempLagged1*TotDASqKM + airTempLagged1*WetlandOrWater + 
     airTempLagged1*log(ImpoundmentsOpenSqKM+0.001) + airTempLagged1*log(ImpoundmentsOpenSqKM+0.001) + airTempLagged1*sweLS + 
     
     airTempLagged2 + airTempLagged2*ReachSlopePCNT + airTempLagged2*TotDASqKM + airTempLagged2*WetlandOrWater + 
     airTempLagged2*log(ImpoundmentsOpenSqKM+0.001) + airTempLagged2*log(ImpoundmentsOpenSqKM+0.001) + airTempLagged2*sweLS + 
     ...
     Forest*daylS + Forest*sradS + Forest*sweLS +
...
if(validate){
  vm4S2 <- validateModel( m4S2,et2 )
  makeBiasMap(vm4S2)
}



#Model 4: No interactions. No Daymet data.
#---------------------------------------------------------------------
m5S2 <- lm(temp~(airTemp+airTempLagged1+airTempLagged2+
              #segment+
              LatitudeS+LongitudeS+
              ForestS+ AgricultureLS+ 
              BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
...
if(validate){
  modelMetricsS2 <- rbind(vm0S2$means, vm1S2$means, vm2S2$means, vm3S2$means, vm4S2$means, vm5S2$means)
  rownames(modelMetricsS2) <- c('vm0S2', 'vm1S2', 'vm2S2', 'vm3S2', 'vm4S2', 'vm5S2')
  modelMetricsS2 <- as.data.frame(modelMetricsS2)
  modelMetricsS2$AIC.df <- AIC(m0S2,m1S2,m2S2,m3S2, m4S2, m5S2)$df
  modelMetricsS2$AIC <- AIC(m0S2,m1S2,m2S2,m3S2, m4S2, m5S2)$AIC
  
  modelMetricsS2
} else ( AIC(m0S2,m1S2,m2S2,m3S2, m4S2, m5S2))
...
if(validate){
  ...
  modelMetricsS3
} else(AIC(m0S3,m1S3,m2S3,m3S3))

rbind(modelMetricsS2, modelMetricsS3  )
...
if(validate){
  ...
  save(segmentModelMetrics, file=paste0(dataOutDir, sourceList,'/segmentModelMetrics.R' ) )
}
...
#predictionStats$bp2 <- predict(finalModBP2,newdata=predictionStats)#,REform=~(1|year))
#predictionStats$bp3 <- predict(finalModBP3,newdata=predictionStats)#,REform=~(1|year))
