...
removeSelectSites <- T
sitesToRemove <- paste0(baseDir, 'GitHub/projects/temperatureProject/dataIn/sitesToRemoveAllNE.csv')
...
```{r scale the covariate data}
#Scale the variables used in the model. Some get log-scaled depending on their distribution over the sites.


# Standard scaling:
et$LatitudeS              <- (et$Latitude        - mean(et$Latitude       , na.rm=T)) / sd(et$Latitude       , na.rm=T)
et$LongitudeS             <- (et$Longitude       - mean(et$Longitude      , na.rm=T)) / sd(et$Longitude      , na.rm=T)
et$ForestS                <- (et$Forest          - mean(et$Forest         , na.rm=T)) / sd(et$Forest         , na.rm=T)
et$BasinElevationMS       <- (et$BasinElevationM - mean(et$BasinElevationM, na.rm=T)) / sd(et$BasinElevationM, na.rm=T)
et$ReachSlopePCNTS        <- (et$ReachSlopePCNT  - mean(et$ReachSlopePCNT , na.rm=T)) / sd(et$ReachSlopePCNT , na.rm=T)
et$WetlandOrWaterS        <- (et$WetlandOrWater  - mean(et$WetlandOrWater , na.rm=T)) / sd(et$WetlandOrWater , na.rm=T)

# Log scaling:
et$AgricultureLS          <- (log(et$Agriculture          + 0.001) - mean(log(et$Agriculture          + 0.001), na.rm=T)) / sd(log(et$Agriculture          + 0.001), na.rm=T)
et$TotDASqKMLS            <- (log(et$TotDASqKM            + 0.001) - mean(log(et$TotDASqKM            + 0.001), na.rm=T)) / sd(log(et$TotDASqKM            + 0.001), na.rm=T)
et$SurficialCoarseCLS     <- (log(et$SurficialCoarseC     + 1    ) - mean(log(et$SurficialCoarseC     + 1    ), na.rm=T)) / sd(log(et$SurficialCoarseC     + 1    ), na.rm=T)
et$ImpoundmentsOpenSqKMLS <- (log(et$ImpoundmentsOpenSqKM + 1    ) - mean(log(et$ImpoundmentsOpenSqKM + 1    ), na.rm=T)) / sd(log(et$ImpoundmentsOpenSqKM + 1    ), na.rm=T)
...
if(validate){
  vm0S2 <- validateModel( m0S2, et2, valData )
  #makeBiasMap(vm0S2)
}
...
if(validate){
  vm1S2 <- validateModel( m1S2, et2, valData )
  #makeBiasMap(vm1S2)
}
...
if(validate){
  vm2S2 <- validateModel( m2S2, et2, valData )
  #makeBiasMap(vm2S2)
}
...
#---------------------------------------------------------------------
m3S2 <- lm(temp~airTemp + airTempLagged1 + airTempLagged2 + LatitudeS + LongitudeS + BasinElevationMS + ReachSlopePCNTS + 
...
if(validate){
  vm3S2 <- validateModel( m3S2, et2, valData )
  #makeBiasMap(vm3S2)
}
...
#---------------------------------------------------------------------
m4S2 <- lm(temp~(airTemp+airTempLagged1+airTempLagged2+
              #segment+
              LatitudeS+LongitudeS+
              ForestS+ AgricultureLS+ 
              BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
...
if(validate){
  vm4S2 <- validateModel( m4S2, et2, valData )
  #makeBiasMap(vm4S2)
}
...
if(validate){
  modelMetricsS2 <- rbind(vm0S2$means, vm1S2$means, vm2S2$means, vm3S2$means, vm4S2$means)
  rownames(modelMetricsS2) <- c('vm0S2', 'vm1S2', 'vm2S2', 'vm3S2', 'vm4S2')
  modelMetricsS2 <- as.data.frame(modelMetricsS2)
  modelMetricsS2$AIC.df <- AIC(m0S2,m1S2,m2S2,m3S2, m4S2)$df
  modelMetricsS2$AIC <- AIC(m0S2,m1S2,m2S2,m3S2, m4S2)$AIC
  
  modelMetricsS2
} else ( AIC(m0S2,m1S2,m2S2,m3S2, m4S2))
...
#Model 3: Selected interactions
#---------------------------------------------------------------------
m3S3 <- lm(temp~airTemp + airTempLagged1 + airTempLagged2 + LatitudeS + LongitudeS + BasinElevationMS + ReachSlopePCNTS + 
     TotDASqKMLS + SurficialCoarseCLS + ForestS + AgricultureLS + WetlandOrWaterS + ImpoundmentsOpenSqKMLS +
     daylS + sradS + sweLS + 
...
#Model 4: Two-way interactions  without Daymet data.
#---------------------------------------------------------------------
m4S3 <- lm(temp~(airTemp+airTempLagged1+airTempLagged2+
              #segment+
              LatitudeS+LongitudeS+
              ForestS+ AgricultureLS+ 
              BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
...
if(validate){
  vm4S3 <- validateModel( m4S3, et3, valData )
  #makeBiasMap(vm4S3)
}
...
#-------------------
if(validate){
  modelMetricsS3 <- rbind(vm0S3$means, vm1S3$means, vm2S3$means, vm3S3$means, vm4S3$means)
  rownames(modelMetricsS3) <- c('vm0S3', 'vm1S3', 'vm2S3', 'vm3S3', 'vm4S3')
  modelMetricsS3 <- as.data.frame(modelMetricsS3)
  modelMetricsS3$AIC.df <- AIC(m0S3,m1S3,m2S3,m3S3,m4S3)$df
  modelMetricsS3$AIC <- AIC(m0S3,m1S3,m2S3,m3S3,m4S3)$AIC
  
  modelMetricsS3
} else(AIC(m0S3,m1S3,m2S3,m3S3,m4S3))
...
valData[,c('pred', 'lowr', 'upr')] <- predict (finalModelS2, newdata=valData, interval = "confidence") #default confidence interval (CI) = 95%


ggplot(valData, aes(temp,pred))+
...
valData[valData$site == 'MEDMR_4BEAVER1.46',]
...
siteData$LatitudeS <- (siteData$Latitude-mean(siteData$Latitude,na.rm=T))/sd(siteData$Latitude,na.rm=T)
siteData$LongitudeS <- (siteData$Longitude-mean(siteData$Longitude,na.rm=T))/sd(siteData$Longitude,na.rm=T)
siteData$ForestS <-   (siteData$Forest-mean(siteData$Forest,na.rm=T))/sd(siteData$Forest,na.rm=T)

siteData$AgricultureLS <-   (log(siteData$Agriculture + 0.001) -mean(log(siteData$Agriculture + 0.001),na.rm=T))/sd(log(siteData$Agriculture + 0.001),na.rm=T)

siteData$BasinElevationMS <-   (siteData$BasinElevationM-mean(siteData$BasinElevationM,na.rm=T))/sd(siteData$BasinElevationM,na.rm=T)
siteData$ReachSlopePCNTS <-   (siteData$ReachSlopePCNT-mean(siteData$ReachSlopePCNT,na.rm=T))/sd(siteData$ReachSlopePCNT,na.rm=T)

siteData$TotDASqKMLS <-   (log(siteData$TotDASqKM+0.001) -mean(log(siteData$TotDASqKM+0.001),na.rm=T))/sd(log(siteData$TotDASqKM+0.001),na.rm=T)

siteData$WetlandOrWaterS <-   (siteData$WetlandOrWater-mean(siteData$WetlandOrWater,na.rm=T))/sd(siteData$WetlandOrWater,na.rm=T)
siteData$SurficialCoarseCLS <-   (log(siteData$SurficialCoarseC + 1) -mean(log(siteData$SurficialCoarseC + 1),na.rm=T))/sd(log(siteData$SurficialCoarseC + 1),na.rm=T)

siteData$ImpoundmentsOpenSqKMLS <-   (log(siteData$ImpoundmentsOpenSqKM+1) -mean(log(siteData$ImpoundmentsOpenSqKM+1),na.rm=T))/sd(log(siteData$ImpoundmentsOpenSqKM+1),na.rm=T)
...
m6bp1 <- lmer(springBP~
              ...
              ForestS+ AgricultureLS+ 
              BasinElevationMS+ ReachSlopePCNTS+ TotDASqKMLS+ 
              WetlandOrWaterS+ SurficialCoarseCLS)^2 +
              (1|year),
         data=siteData)
...
catchments <- readShapePoly ( "C:/KPONEIL/USGS/NHDPlusV2/Modified Versions/MEDMR_Region_NHDCatchment.shp", proj4string=CRS(proj4.NHD))
...
predictionStats$bp1 <- predict(finalModBP1,newdata=predictionStats, allow.new.levels = T)#,REform=~(1|year))
predictionStats$bp2 <- predict(finalModBP2,newdata=predictionStats, allow.new.levels = T)#,REform=~(1|year))
predictionStats$bp3 <- predict(finalModBP3,newdata=predictionStats, allow.new.levels = T)#,REform=~(1|year))
...
for ( i in 1:length(DaymetTiles)){
  ...
  FR2day <- data.frame( FR2[,c('site', 'year.x', 'dOY')], sweLS = (FR2$sweL-mean(FR2$sweL,na.rm=T))/sd(FR2$sweL,na.rm=T), daylS = (FR2$dayl-mean(FR2$dayl,na.rm=T))/sd(FR2$dayl,na.rm=T), sradS = (FR2$srad-mean(FR2$srad,na.rm=T))/sd(FR2$srad,na.rm=T))
  
  FR3day <- data.frame( FR3[,c('site', 'year.x', 'dOY')], sweLS = (FR3$sweL-mean(FR3$sweL,na.rm=T))/sd(FR3$sweL,na.rm=T),              daylS = (FR3$dayl-mean(FR3$dayl,na.rm=T))/sd(FR3$dayl,na.rm=T), sradS = (FR3$srad-mean(FR3$srad,na.rm=T))/sd(FR3$srad,na.rm=T))
