...
locations <- read.csv(paste0(baseDir, '/MADEP/dataIn/tblLocationDetails_Multi.csv'))

agency <- 'MADEP'

setwd(paste0(baseDir, '/MADEP/dataIn/temperatureFiles/Multi-Probes'))
files <- list.files()
...
for( i in 1:length(files)){
  ...
  siteData <- data.frame( site = paste0(agency, '_', agencyID), dataIn, 
                          Latitude = unique(locations$Latitude[locations$UNIQUE_ID == agencyID]), 
                          Longitude = unique(locations$Longitude[locations$UNIQUE_ID == agencyID]), 
...
finalData$temp <- as.numeric(paste(finalData$Temperature.......................Daily.Mean))
...
# This section is needed because of receiving only pieces of the data at a time
```{r Merge in with existing data}

finalData$probeType <- 'M1'
finalData$site <- paste0(finalData$site, '_', finalData$probeType)

load(paste0('C:/KPONEIL/GitHub/projects/temperatureProject/dataIn/MADEP/streamTempData_MADEP.RData'))

masterData <- rbind(finalData, masterData)
...
siteLocs <- unique(masterData[,c('site', 'Latitude', 'Longitude', 'agency')])
