...
for (k in 1:length(Tiles)){
  ...
  save(FullRecord, file = paste0("C:/KPONEIL/USGS/Stream Temperature/data/temperature/fromKyle/BP_Analysis/BP_Analysis/DaymetClimateData/LocalValues/NHD_DaymetTile_", Tiles[k],"_", year, ".RData"))
...
```{r Upstream Precipitation Values}
...
```{r Define section name:}


NEday2010 <- merge(LocalRecord, UpstreamRecord, by = c("site", "year", "dOY"), all.x = T,  sort = F))
NEday2010$airTemp <- (NEday2010$tmin + NEday2010$tmax)/2


names(NEday2010)[]


save(NEday2010, file = "C:/KPONEIL/USGS/Stream Temperature/data/temperature/NEdaymet2010.RData")



UpRes <- FullRecord[,c(1,2,3,8)]



load("C:/KPONEIL/USGS/Stream Temperature/data/temperature/fromKyle/BP_Analysis/BP_Analysis/BPA_ClimateData865.RData")
master.data <- merge(master.data, UpRes, by = c('site', 'year', 'dOY'), all.x = T)
