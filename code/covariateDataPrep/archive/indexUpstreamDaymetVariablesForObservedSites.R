...
for (i in 1:numTiles){
  ...
  #--------------------------------------------
  NCDF <- open.ncdf(paste0("F:/KPONEIL/SourceData/climate/DAYMET/unzipped/Daily/", Tiles[i], "_2010/prcp.nc"))    #netcdf   
  ...
  for (j in 1:length(Variables)){
    ...
    for ( year in BegYr:EndYr ){
      ...
      for ( t in 1:length(SubTiles)){
        ...
        NCDF <- open.ncdf(paste0("F:/KPONEIL/SourceData/climate/DAYMET/unzipped/Daily/", SubTiles[t], "_", year,"/", Variables[j], ".nc"))    #netcdf
        #Dimension limits of each of the variables we'll use:
        #----------------------------------------------------
        start1 = c(1,1)
        latcount <- c(NCDF$var$lat$varsize[1], NCDF$var$lat$varsize[2])
