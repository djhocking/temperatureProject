...
########################## NEED TO INCORPORATE TO THISE VERSION ##############
#Use validation?
validate = T
...
if(validate) {
  n.fit <- floor(length(unique(tempDataSync$site)) * (1 - p.valid))
...
tempDataSyncS <- cbind(tempDataSync[ ,c(1:13)],
             apply(X = tempDataSync[ ,14:dim(tempDataSync)[2]], MARGIN=2,
...
tempDataSyncValidS <- cbind(tempDataSyncValid[ ,c(1:13)],
             apply(X = tempDataSyncValid[ ,14:dim(tempDataSyncValid)[2]], MARGIN=2,
                   FUN = function(x){(x-mean(x, na.rm=TRUE)) / sd(x, na.rm=TRUE)}))
